...
class KvLite {
	...
	protected function connect(){
    	...
    	try {
    		...
    		if( ! in_array('kv', $tables)) {
    			if($this->db->exec('CREATE TABLE [kv] (id, key, value)') === FALSE) die(print_r($this->db->errorInfo(), true));;
    ...
    public static function get($id, $key = FALSE) {
		$rows = $dbdata = FALSE;
		$kvlite = self::getInstance();
		$dbdata = $kvlite->dbGet($id, $key);
		if(is_array($dbdata)){
    ...
    public static function dbGet($id, $key = FALSE) {
        ...
        $sql = "SELECT * FROM kv WHERE id = :id $key";
    ...
    public function delete($id,$key){
    	$sql="DELETE FROM [kv] WHERE id=? AND key= ?";
		$sth = $this->db->prepare($sql);
		return $sth->execute(array($id, $key));
    }
    ...
    public static function save($id,$key,$value){
    	...
    	$sql="SELECT count(*) FROM [kv] WHERE id == ? AND key == ? ";
		$sth = $kv->db->prepare($sql);
		$sth->execute(array($id, $key));;
		$rows = $sth->fetch(PDO::FETCH_NUM);
    	if ($rows[0]>0){
    		return $kv->update($id,$key,$value);
    	}else{
    		return $kv->insert($id,$key,$value);
    	}
    }
	protected function insert($id,$key,$value){
		$sql = "INSERT INTO [kv] (id,key,value) VALUES (?,?,?)";
		$sth = $this->db->prepare($sql);
		return $sth->execute(array($id, $key, $value));
	}
	protected function update($id,$key,$value){
		$sql = "UPDATE [kv] SET value = :value WHERE id = :id AND key = :key";
		$sth = $this->db->prepare($sql);
		return $sth->execute(array(':id'=>$id,':key'=>$key,':value'=>$value));
	}
	 ...
	 */
	public static function insertArray($id,$array){
	...
	protected function numRows($id){
		$sql="SELECT count(*) FROM [kv] WHERE  id == ? ";
		$sth = $this->db->prepare($sql);
