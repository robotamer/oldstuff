...
class Session {
    ...
    private $lifetime = 1; // Day // Move to config
...
class DB_Session {
    ...
    public static function open() {
        $parm = (defined('ROOT')) ? 'sqlite:'.ROOT.'/data/tmp.db3' : 'sqlite:/tmp/session.db3';
        $tables = array();
        global $db_session;
    ...
    public static function write($id, $data) {
        ...
        $return = $sth->execute(array(':id'=>$id, ':name'=> session_name(), ':data'=>$data, ':fingerprint'=>md5($_SERVER['HTTP_USER_AGENT'].self::$salt))); //Returns TRUE on success or FALSE on failure.
        return $return;
    }
    ...
    public static function destroy() {
        ...
        if( ! is_object($db_session) ) self::open();
        $id = session_id();
        $count = $db_session->exec("DELETE FROM sessions WHERE id = '$id'");
        return $count;
    }
    /**
