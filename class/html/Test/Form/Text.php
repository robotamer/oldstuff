<?php
/**
 * $Id: FormText.class.php 562 2006-09-28 16:38:53Z matthieu $
 */
if (!class_exists('Test_Form_Text')) {
	if (!defined('__CLASS_PATH__')) {
		define('__CLASS_PATH__', realpath(dirname(__FILE__) . '/../../'));
	}
	require_once __CLASS_PATH__ . '/Autoload.php';
	/**
	 * unit test case for text class
	 * @author matthieu <matthieu@phplibrairies.com>
	 * @package html
     * @subpackage unit_test_case
	 */
	class Test_Form_Text extends Test_Html_Template implements Test_Html_Design_Interface {
		/**
		 * the test field name
		 */
		...
		public function setUp() {
			$this->obj = new Form_Text($this->fieldname);
		}
		/**
		...
		public function testDefault() {
			$this->_testCode('<input type="text" name="' . $this->fieldname . '"/>');
		}
		/**
		 * test the set style method
		 * @access public
		...
		public function testSetStyle() {
			...
			$this->_testCode('<input type="text" name="' . $this->fieldname . '" style="' . parent :: __STYLE_CONTENT__ . '"/>');
		...
		public function testSetStyleClass() {
			...
			$this->_testCode('<input type="text" name="' . $this->fieldname . '" class="' . parent :: __STYLE_CLASS_NAME__ . '"/>');
		...
		public function testSetValue() {
			...
			$this->_testCode('<input type="text" name="' . $this->fieldname . '" value="' . $this->value . '"/>');
		}
		/**
		...
		public function testSetLength() {
			...
			$this->_testCode('<input type="text" name="' . $this->fieldname . '" size="' . self :: __LENGTH__ . '"/>');
		}
		/**
		 * test the set length method
		 * @access public
		...
		public function testSetMaxLength() {
			...
			$this->_testCode('<input type="text" name="' . $this->fieldname . '" maxlength="' . self :: __LENGTH__ . '"/>');
		}
		/**
		...
		public function testAddJS() {
			...
			$this->obj->addJs($js);
			$this->_testCode('<input type="text" name="' . $this->fieldname . '" ' . $js->__toString() . '/>');
			unset ($js);
		}
		/**
		...
		*/
		public function testSetDisabled() {
			try {
				...
				$this->_testCode('<input type="text" name="' . $this->fieldname . '" disabled/>');
			}
			catch (Exception $e) {
